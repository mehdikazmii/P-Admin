...
import 'package:betting_admin/screens/bottomnavigation/bet_screens/edit_bet_screen.dart';
import 'package:betting_admin/screens/bottomnavigation/bet_screens/report_screen.dart';
...
class _BetCardScreenState extends State<BetCardScreen> {
  ...
  removeBet() {
    ...
    try {
      ...
      snapshots.forEach((elements) {
        for (var element in elements.docs) {
          returnPoints(element);
        }
      });
      instance.collection('publish').doc(widget.data.id).delete();
      setState(() {
        isLoading = false;
      });
      Navigator.pop(context);
      showSnackBar('bet has been removed and points are returned', context);
    } catch (e) {
      print(e);
      showSnackBar(e.toString(), context);
    }
  ...
  returnPoints(element) {
    try {
      instance.collection('users').doc(element.id).get().then((value) {
        double wallet = value['wallet'] + int.parse(element['bet']);
        instance
            .collection('users')
            .doc(element.id)
            .update({'wallet': wallet}).then((value) {
          print('points have been returned');
        });
      });
    } catch (e) {
      print(e.toString());
